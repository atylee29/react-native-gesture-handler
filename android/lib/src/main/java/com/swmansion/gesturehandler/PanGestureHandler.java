...
public class PanGestureHandler extends GestureHandler<PanGestureHandler> {
  ...
  private float mFailSlope = MIN_VALUE_IGNORE;
  ...
  public PanGestureHandler setFailSlope(float failSlope) {
    mFailSlope = failSlope;
    return this;
  }
  ...
  private boolean shouldFail() {
    ...
    if(mFailSlope != MIN_VALUE_IGNORE) {
      if(dx == 0) {
        return dy != 0;
      } else {
        float slope = dy / dx;
        return slope >= 0 ? slope < mFailSlope : -slope < mFailSlope;
      }
    }
    return false;
  }

...
@property (nonatomic) CGFloat failSlope;
...
- (void)updateHasCustomActivationCriteria
{
  ...
  ||  !isnan(_activeOffsetYStart) || !isnan(_activeOffsetYEnd) || !isnan(_failSlope);
}
...
- (BOOL)shouldFailUnderCustomCriteria
{
  ...
  if(!isnan(_failSlope)) {
      if(trans.x == 0) {
          ...
          if(fabs(trans.y)/fabs(trans.x) > _failSlope) {
              return YES;
          }
      }
  }
